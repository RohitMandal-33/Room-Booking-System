...
class ReportRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllReports(): Result<ReportPageDTO> = runCatching {
        val response = api.getAllReports()
        if (!response.success || response.data == null) throw Exception(response.message)
        response.data
    }

    override suspend fun getFilteredReports(request: ReportDataRequestDTO): Result<ReportPageDTO> = runCatching {
